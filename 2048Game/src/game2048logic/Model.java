...
public class Model {
     ...
     * */
    public boolean emptySpaceExists() { // board: use size() and tile(int x, int y)
        for (int x = 0; x < size(); x++) {
            for (int y = 0; y < size(); y++) {
                if ((tile(x, y)) == null) { // use 'if (t==null)'
                    return true;
                }
            }
        }
     ...
     */
    public boolean maxTileExists() { //use 'MAX_PIECE' instead of '2048' to avoid problem
        for (int x = 0; x < size(); x++) {
            for (int y = 0; y < size(); y++) {
                Tile T = tile(x, y);
                if (T != null) { //!=not
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int x = 0; x < size(); x++) {
            for (int y = 0; y < size(); y++) {
                ...
                if (T != null) {
                    if (x < size - 1) {
                        Tile adjacent = tile(x + 1, y);
                        if (adjacent != null) {
                            if (T.value() == adjacent.value()) {
                                return true;
                            }
                        }
                    }
                    if (y < size - 1) {
                        Tile next = tile(x, y + 1);
                        if (next != null) {
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        while (newY < board.size()) {
            Tile next = board.tile(x, newY);

            if (next == null) {
                targetY = newY;
            } else if (next.value() == myValue) {
                if (!next.wasMerged()) {
    ...
    public void tilt(Side side) { //use setViewingPerspective(Side s)
        board.setViewingPerspective(side);
        for (int x = 0; x < board.size(); x++) {
            tiltColumn(x);
        }
        board.setViewingPerspective(Side.NORTH); //set perspective back to north before calling tilt
    }
