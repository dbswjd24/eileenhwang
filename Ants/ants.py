...
class Ant(Insect):
    ...
    def double(self):
        ...
        if self.is_container and self.ant_contained!= None: 
            if self.ant_contained.is_doubled == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cnt = 0
        check_place = self.place # Start from Current Place
        while check_place.entrance:                 
            if cnt >= self.lower_bound and cnt <= self.upper_bound:
                if not check_place.is_hive:
                    if len(check_place.bees) > 0:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        [h.reduce_health(amount) for h in self.place.bees[:]]
        [d.reduce_health(self.damage) for d in self.place.bees[:] if self.health <= amount]
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        [Ant.reduce_health(h, self.damage) for h in self.place.bees[:]]
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        positions = self.place.exit
        while positions:
            if positions.ant != None:
                positions.ant.double()
            positions = positions.exit
