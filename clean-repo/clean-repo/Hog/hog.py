...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    """
    # BEGIN PROBLEM 4
    current = simple_update(num_rolls, player_score, opponent_score, dice)
    return sus_points(current)
    # END PROBLEM 4
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(goal):
        for opponent_score in range(goal):
            if strategy(score, opponent_score) !=start:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    max_num_rolls, max_score = 1,0
    for num_rolls in range (1,11):
        score = averaged_roll_dice(num_rolls, dice)
        if score > max_score:
            max_num_rolls, max_score = num_rolls, score
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    susi = sus_update(0, score, opponent_score)
    if susi - score >= threshold:
        return 0
    else: 
        return num_rolls
    # END PROBLEM 11
